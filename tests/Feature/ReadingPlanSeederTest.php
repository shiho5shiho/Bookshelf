...
class ReadingPlanSeederTest extends TestCase
{
    ...
    public function test_in_progressのデータが6件作成される(): void
    {
        // Arrange
        $this->seedPrerequisites();
        ...
        // Assert
        $this->assertSame(6, ReadingPlan::where('status', ReadingPlanStatus::InProgress)->count());
    }
    ...
    public function test_expiredのデータが1件作成される(): void
    {
        // Arrange
        $this->seedPrerequisites();
        ...
        // Assert
        $this->assertSame(1, ReadingPlan::where('status', ReadingPlanStatus::Expired)->count());
    }
    ...
    public function test_鈴木花子の期日はリマインダー_失効どちらの条件にも一致しない(): void
    {
        ...
        $this->assertNotEquals($today->copy()->subDays(3)->toDateString(), $suzukiPlan->target_date->toDateString());
        $this->assertFalse($suzukiPlan->target_date->lt($today->copy()->subDays(3)));
    }
